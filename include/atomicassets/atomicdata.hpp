...
namespace atomicdata {
    ...
    vector <uint8_t> inline toVarintBytes(uint64_t number, uint64_t original_bytes = 8) {
        if (original_bytes < 8) {
    ...
    uint64_t inline unsignedFromVarintBytes(vector <const uint8_t>::iterator &itr) {
        uint64_t number = 0;
    ...
    vector <uint8_t> inline toIntBytes(uint64_t number, uint64_t byte_amount) {
        vector <uint8_t> bytes = {};
    ...
    uint64_t inline unsignedFromIntBytes(vector <const uint8_t>::iterator &itr, uint64_t original_bytes = 8) {
        uint64_t number = 0;
        uint64_t multiplier = 1;
    ...
    uint64_t inline zigzagEncode(int64_t value) {
        if (value < 0) {
    ...
    int64_t inline zigzagDecode(uint64_t value) {
        if (value % 2 == 0) {
    ...
    vector <uint8_t> inline serialize_attribute(const string &type, const ATOMIC_ATTRIBUTE &attr) {
    ...
    ATOMIC_ATTRIBUTE inline deserialize_attribute(const string &type, vector <const uint8_t>::iterator &itr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            //Type is an array
    ...
    vector <uint8_t> inline serialize(ATTRIBUTE_MAP attr_map, const vector <FORMAT> &format_lines) {
        uint64_t number = 0;
        vector <uint8_t> serialized_data = {};
    ...
    ATTRIBUTE_MAP inline deserialize(const vector <uint8_t> &data, const vector <FORMAT> &format_lines) {
